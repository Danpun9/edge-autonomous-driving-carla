"""
File: step16_sensor_fusion_drive.py

Purpose:
    Combine U-Net lane perception and YOLO object detection for CARLA driving.

Main Responsibilities:
    - Load AdvancedUNet and YOLO models.
    - Estimate lane center, traffic-light state, and lead vehicles.
    - Fuse perception outputs into path following, ACC, and braking behavior.

Notes:
    Requires CARLA and trained weights. This is an interactive driving demo.
"""
...
def main():
    ...
    try:
        ...
        print("=== Step 16: Sensor Fusion Drive Started ===")
        ...
        while True:
            ...
            lane_pixels = np.argwhere(pred_mask == 1) # (y, x) 반환
            lane_center_x = IM_WIDTH // 2 # 초기값 (화면 중앙)

            if len(lane_pixels) > 0:
                # 차선 픽셀 중 가장 위쪽(Y가 가장 작은) 15픽셀의 X좌표 평균 = 소실점
                min_y = np.min(lane_pixels[:, 0])
                top_pixels = lane_pixels[lane_pixels[:, 0] < min_y + 15]
                lane_center_x = int(np.mean(top_pixels[:, 1]))

            # 시각화: U-Net이 지목한 차선의 수직선(Ray) 그리기
            cv2.line(vis_frame, (lane_center_x, 0), (lane_center_x, IM_HEIGHT), (255, 255, 0), 1, cv2.LINE_AA)
            cv2.putText(vis_frame, "U-Net Ego-Lane Ray", (lane_center_x + 5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)

            # ---------------------------------------------------------
            # [Phase 2 & 3: YOLO 탐지 및 수직선 매칭 (Ray Casting)]
            # ---------------------------------------------------------
            # class 9: traffic light
            yolo_results = yolo_model.predict(source=frame_bgr, conf=0.3, classes=[9], verbose=False)[0]
            ...
            for box in yolo_results.boxes:
                ...
                # 가상 수직선(lane_center_x)과의 X축 거리 계산
                dist = abs(tl_center_x - lane_center_x)
                
                # 거리가 80픽셀 이내일 때만 내 차선의 신호등으로 인정
            ...
            # ---------------------------------------------------------
            # [Phase 4: 위계적 융합 제어 (Hierarchical Control)]
            # ---------------------------------------------------------
            ...
            # [핵심] 센서 퓨전 오버라이드 (적색 신호 시 조향 유지 & 강제 제동)
            if tl_state == "RED":
                cv2.putText(vis_frame, "EMERGENCY BRAKE: RED LIGHT", (IM_WIDTH//2 - 200, IM_HEIGHT - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
